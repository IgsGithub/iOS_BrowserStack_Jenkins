...
//	}
	...
	public List<WebElement> findElements(By byLocator) {
		if (getPlatform().equals("Android") || getPlatform().equals("MPWA") ||getPlatform().equals("HIPI_iOS")) {
...
//							"Swiping screen in " + " " + dire + " direction" + " " + (j + 1) + " times");
	...
	public void TimeStamptapOnScrreenTocloseCommentScreen(int k) {
		@SuppressWarnings("rawtypes")
		TouchAction touchAction=new TouchAction(getDriver());
	...
	public void TimeStampPartialCommentsSwipeiOS(String direction, int count) {
		touchAction = new TouchAction(getDriver());
		String dire = direction;
	...
	public static boolean TimeStampverifyIsElementDisplayed(By by, String validationtext) {
		//getDriver().manage().timeouts().implicitlyWait(2, TimeUnit.SECONDS);
		List<WebElement> list = getDriver().findElements(by);
		//getDriver().manage().timeouts().implicitlyWait(30, TimeUnit.SECONDS);
		if (list.size() == 0) {
	...
	public boolean TimeStampwaitForElementNotDisplayed(By byLocator,int k,String validationtext) {
		for(int i=0;i<=k;i++){
			if(TimeStampverifyIsElementDisplayed(byLocator, validationtext)){
				System.out.println(validationtext+" is displayed");
				
			}else{
				System.out.println(validationtext+" not displayed");
				break;				
			}
		}
		return true;
	...
	public void tapiOSAtCoOrdinates(int x, int y, int k) {
		@SuppressWarnings("rawtypes")
		TouchAction touchAction=new TouchAction(getDriver());
